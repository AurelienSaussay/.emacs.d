...
;; Copyright (C) 2012 Sylvestre Ledru
...
(defconst scilab-mode-version "3.0"
...
(defvar scilab-mode-map
  (let ((km (make-sparse-keymap)))
    ...
    (define-key km [(meta control g)] 'scilab-function-goto-line)
...
Travels across continuations."
  ...
  (let ((p nil)
    ...
    (while (and (or (save-excursion (and (scilab-prev-line)
		...
		(save-excursion (forward-line 0) (not (bobp))))
...
;;	       (paragraph-start    "//[a-zA-Z]\\|//[ \t]*$\\|[ \t]*$")
	   ...
	   (scilab-set-comm-fill-prefix)
           (save-excursion
	     (setq here (point))
	     (setq par-start (re-search-backward paragraph-start nil t))
             (if (not par-start)
                 nil
		 (forward-line 0)
	     ...
	     (goto-char here)
	     (re-search-backward "^[^/]*\\([^/]/\\([^/]\\|$\\)\\|[^/]\\)+$" nil t)
	     (setq nocomment (point))
	     (if (or (not par-start) (>= nocomment par-start))
		 (progn
		   (next-line 1)
...
(defun scilab-find-recent-variable-list (prefix)
  "Instead of looking through the session we just use Scilab who command to
get all variables"
(interactive)
...
value is contained in the customized variable `scilab-libfunc-list-path'.
 The function first checks if the file exisits. If not it checks if $SCIHOME/libfunc file exists. If non of this exists and shell is active it builds this file. Otherwise it returns nil"
  ...
  (if (or (not (scilab-shell-active-p)) (not (scilab-on-prompt-p)))
      nil
 ...
 (easy-menu-define scilab-mode-menu
   ...
   '("Scilab"
     ...
     ["Save and getf" scilab-shell-save-and-getf-or-run
                ...
                :active ;;added for compatibility of old easy-menu
...
(defcustom scilab-shell-command
  ...
  You can use standard \"scilab\" utilities or your own
...
;	      (define-key km [down] 'comint-next-matching-input-from-input)
    ...
    (switch-to-buffer
     (if (fboundp 'make-comint-in-buffer)
	 (make-comint-in-buffer scilab-shell-buffer-name nil
				scilab-shell-command
				nil scilab-shell-command-switches)
	 (make-comint  scilab-shell-buffer-name
				scilab-shell-command
				nil scilab-shell-command-switches)))
...
\\{scilab-mode-map}"
  ...
  (message "Waiting for scilab process ready...")
  (goto-char (point-max))
...
This command does not require an active Scilab shell."
  (interactive (list
		(let* ((default  (scilab-read-word-at-point))
		      (default (if (not (string= default "")) default nil)))
		  (if (and (not scilab-scifunctions-list) (scilab-shell-active-p))
...
(defun scilab-on-prompt-p ()
 ...
 (save-excursion
    ...
    (if (looking-at comint-prompt-regexp)
        (progn
	  (previous-line 1)
	  (if (not (looking-at comint-prompt-regexp)) t nil))
      nil)))
...
;    (if (file-exists-p (concat scilab-shell-initial-directory "/man/"))
 ;   (setq chaptername (concat scilab-shell-initial-directory "/man/Chapters"))
  ...
  (if (not (file-exists-p chaptername))
    (error"No manual Chapters file. Run scilab-shell, or build manually")
    )))
...
(while  (and  (> (length lst) 0) (not success))
   ...
   (if success
       (progn
	 (switch-to-buffer-other-window  bfn)
   ...
   (list
    (let ((default (save-excursion
		     ...
		     (scilab-navigation-syntax
		       (if (looking-at "\\sw+\\>")
			   (match-string 0))))))
      (if (and (not scilab-scifunctions-list)
	       (scilab-shell-active-p)
	       (scilab-on-prompt-p))
	  (setq scilab-scifunctions-list
		(scilab-find-scifunctions-list "" t)))
   ...
   (if (string-match "\\.sci$" funname)
	...
	(if (not pthf) nil
          (if (file-exists-p (concat (substring pthf 0 -3) "sci"))
	       ...
	       (scilab-find-function-in-file funname))
             (if (not (file-exists-p (concat pth scilab-lib-tag-table-name)))
                 nil
	       (if (string-match "^\\s-*\\*Minibuf" (buffer-name (current-buffer)))
...
the paths given in `scilab-help-pages-path' variable"
  (interactive)
  (setq dirs scilab-help-pages-path)
  (setq found nil)
  (setq str nil)
...
;;date: 2012/03/23;  author Sylvestre Ledru

;;; Project moved on the Scilab forge:
;;; http://forge.scilab.org/index.php/p/scilab-emacs/
;;; Fix a typo
;;;
;;; Version 3.0
